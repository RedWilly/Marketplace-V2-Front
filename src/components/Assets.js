...
function Assets() {
    const [nfts, setNfts] = useState([1,2,3,4,5,6,7])
    return (
      <div className='mt-6'>
        ...
        <div className='flex justify-start items-stretch gap-9 flex-wrap sm:gap-3'>
              {nfts?.map((n, index)=> {
                  ...
                  <button className='bg-blue-100 w-full py-2 absolute div -bottom-20 cursor-pointer transition-all ease-linear duration-250'>
                      <p className='text-sm font-Kallisto font-medium uppercase text-center text-white/75 tracking-wider'>{"But Now"}</p>
                  </button>
              </div>
              })}
              {/* {nfts?.map((n, index)=> {
                  return <NFTCard buttonTitle={"But Now"} onClick={()=>{}} key={index} image={''} name={"Hello"} id={index} price={250} last_price={80}/>
              })} */}
        </div>
      </div>
    )
